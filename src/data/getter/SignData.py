...
from model.getter.Sign import Sign
...
class SignData:
    ...
    def __init__(self):
        ...
        self.signs="/api/fichadas"

    def callback(self,json_data):
        signs=[]

        for obj in json_data['data']:
            signs.append(Sign(  obj['id'], obj['codigo_aux'], obj['id_persona'],
    ...
    def getSignFromAPI(self,database,page):
        request=RequestHTTP(base_url=self.base_url)
        dataResponse=request.GET(self.signs, params={ "database" : database , "page" : page },
    ...
    def getTotalPagesFromAPI(self,database):
        request=RequestHTTP(base_url=self.base_url)
        dataResponse=request.GET(self.signs, params={ "database" : database , "page" : "1" },
                    headers={ "token" : self.api_token , "Accept" : "application/json" },
